...
def limit_cache():
    if 'limit_cache_use' not in st.session_state:
        ...
        st.session_state.limit_cache_use = not st.session_state.limit_cache_use
...
def full_reset():
    ...
    if 'input_file_name' in st.session_state:
        del st.session_state.input_file_name    
    if 'example_selector' in st.session_state:
        del st.session_state['example_selector']
    if 'illumination_detail' in st.session_state:
        del st.session_state['illumination_detail']
    if 'power' in st.session_state:      
        del st.session_state['power']
    # # only additional behavior (compared to "reset()") is setting

    if st.session_state.limit_cache_use:
        st.experimental_memo.clear() # clear cache resulting from functions decorated with @st.experimental_memo

    st.session_state.full_clear = True

    # # If true, then clicking "Reset Now" button:
    # #   1.  sets st.session_state.full_clear to False
    # #   2.  triggers call to st.experimental_rerun() , which should cause the app to be rerun from the top
    #   NOTE: can't just call st.experimental_rerun here because "Calling st.experimental_rerun() within a callback is a no-op."
...
#        with st.expander("Console"):
        ...
        with st.expander("Reset App"):
            with st.form("Apply"):
                ...
                if st.session_state.full_clear:  # condition will always be true immediately after calling full_reset(), but always false otherwise
                    if 'example_selector' in st.session_state:
                        del st.session_state['example_selector']
                    if 'illumination_detail' in st.session_state:
                        del st.session_state['illumination_detail']
                    if 'power' in st.session_state:      
                        del st.session_state['power']
                    st.session_state.full_clear = False  # st.experimental_rerun isn't callable from a callback
                    st.experimental_rerun()  # This design effectively restores callability fo st.experimental_rerun in this callback, albeit the call occurs now instead of during the callback execution

        # logging = st.radio("Process Log:", ('OFF', 'ON'), on_click=set_logging, args=)
        input_selection = st.radio("Select Example:", ('scrapyard', 'selfie', 'cylinder', 'park', 'school', 'spiral'), horizontal=True, on_change=reset, key='example_selector')
        ...
        with st.expander("Parameter Settings"):
            with st.form('Parameter Settings'):
                ...
                ################################################################################################
                ################### GUI PARAMETERS #########################################################
                granularity_selection = st.radio("Illumination detail", ('standard', 'boost', 'max'), horizontal=True, key='illumination_detail')
                granularity_dict = {'standard': 0.1, 'boost': 0.3, 'max': 0.5}
                granularity = granularity_dict[granularity_selection]

                power = float(st.text_input(f'Power     (default = {default_power})', str(default_power), key='power'))
    ...
    if 'input_np' not in st.session_state:  # condition true on initial run and immediately after call to reset() function 
                                                #  reset() is the "on_change" callback function called by widgets: st.file_uploader(), st.radio("Select Example:", ...)
                                                # so uploading a new image file or changing the example selection removes the 'input_np' attribute from st.session_state
        if fImage is not None:  # image uploaded by user (via st.file_uploader)
            ...
            st.session_state.input_file_name = IMAGE_EXAMPLE_PATH.split('\\')[-1]  # use example image
            st.session_state.image_np = np.array(ImageOps.exif_transpose(Image.open(IMAGE_EXAMPLE_PATH)))[:,:,[2,1,0]]

    image_np = st.session_state.image_np  # input image as uint8 numpy array in [height, weight, [red, green, blue]] format
    input_file_name = st.session_state.input_file_name  # used to generate file names for optional download
